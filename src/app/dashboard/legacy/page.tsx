...
export default async function Home() {
  const currentUser = await requireUser("/dashboard/legacy");
  if (!canAccessAdminControlTower(currentUser)) {
    redirect(`/access-denied?from=${encodeURIComponent("/dashboard/legacy")}&next=${encodeURIComponent(defaultLandingForRole(currentUser.role))}`);
  }
