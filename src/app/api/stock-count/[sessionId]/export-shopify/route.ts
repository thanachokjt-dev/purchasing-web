...
import { serializeShopifyStockCountCsv } from "@/lib/stock-count-csv";
...
export async function GET(
  ...
  const csv = serializeShopifyStockCountCsv(data.lines);
  const filename = `shopify-stock-${data.session.locationType}-${data.session.weekStart}.csv`;
